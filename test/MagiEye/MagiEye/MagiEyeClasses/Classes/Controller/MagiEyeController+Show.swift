...
extension MagiEyeController {
    ...
    var animating: Bool {
        get {
            return objc_getAssociatedObject(self, &MagiDefine.Key.Associated.Animation) as? Bool ?? false
        }
        
        set {
            objc_setAssociatedObject(self, &MagiDefine.Key.Associated.Animation, newValue, .OBJC_ASSOCIATION_ASSIGN)
        }
    }
    
    var showing: Bool {
        get {
            return objc_getAssociatedObject(self, &MagiDefine.Key.Associated.Showing) as? Bool ?? false
        }
        
        set {
            objc_setAssociatedObject(self, &MagiDefine.Key.Associated.Showing, newValue, .OBJC_ASSOCIATION_ASSIGN)
        }
    }
