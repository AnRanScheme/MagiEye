...
//  Copyright © 2019 anran. All rights reserved.cpu
//
...
/// CPU Class
open class MagiCPU: NSObject {
    
    // MARK: OPEN PROPERTY

    /// Number of physical cores on this machine.
    static var physicalCores: Int {
        get {
            return Int(MagiSystemEye.hostBasicInfo.physical_cpu)
        }
    }
    ...
    static var logicalCores: Int {
        get {
            return Int(MagiSystemEye.hostBasicInfo.logical_cpu)
        }
    }
    ...
    static func systemUsage() -> (system: Double,
        user: Double,
        idle: Double,
        nice: Double) {
    ...
    open class func applicationUsage() -> Double {
        ...
        threads.forEach { (thread:thread_basic_info) in
            if self.flag(thread) {
                result += Double(thread.cpu_usage)/Double(TH_USAGE_SCALE)
            }
        }
        return result * 100
    }
        ...
        get {
            ...
            let result = withUnsafeMutablePointer(to: &hostInfo) {
                $0.withMemoryRebound(to: integer_t.self, capacity: Int(size)) {
                    host_statistics(MagiSystemEye.machHost, HOST_CPU_LOAD_INFO, $0, &size)
                }
            }
    ...
    private class func flag(_ thread: thread_basic_info) -> Bool {
        ...
        return !number.boolValue//Bool.init(truncating: number)
    }
    ...
    private class func threadBasicInfos() -> [thread_basic_info]  {
        ...
        for act_t in self.threadActPointers() {
            ...
            basic_info_th = withUnsafePointer(to: &thinfo.pointee,
                                              { (ptr) -> thread_basic_info_t in
                                                let int8Ptr = unsafeBitCast(
                                                    ptr,
                                                    to: thread_basic_info_t.self)
                                                return int8Ptr
            })
    ...
    private class func threadIdentifierInfos() -> [thread_identifier_info] {
        ...
        for act_t in self.threadActPointers() {
            ...
            identifier_info_th = withUnsafePointer(to: &thinfo.pointee,
                                                   { (ptr) -> thread_identifier_info_t in
                let int8Ptr = unsafeBitCast(
                    ptr,
                    to: thread_identifier_info_t.self)
                return int8Ptr
            })
