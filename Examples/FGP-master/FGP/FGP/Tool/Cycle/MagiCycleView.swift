...
class CycleImageView: UIView {
    ...
    lazy var livingView: UIButton = {
        let btn = UIButton()
        btn.isHidden = true
        
        btn.setTitle("预告", for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        btn.setBackgroundColor(UIColor(hex: 0xff9900),
                               forUIControlState: .normal)
    ...
    override func layoutSubviews() {
        ...
        livingView.frame = CGRect(x: 12,
                                   y: bounds.height - 25,
                                   width: 40,
                                   height: 25)
        titleLabel.frame = CGRect(x: livingView.frame.maxX,
                                  y: bounds.height - 28.0,
                                  width: bounds.width - livingView.frame.maxX,
                                  height: 28.0)
...
public class MagiCycleView: UIView {
    ...
    fileprivate lazy var scrollView: UIScrollView = { [weak self] in
        ...
        if let strongSelf = self {
            ...
            scroll.isScrollEnabled = true // strongSelf.pageCount > 1 ? true : false
        }
...
extension MagiCycleView: UIScrollViewDelegate {
    ...
    public func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        // 1/2屏
        if abs(scrollView.contentOffset.x - scrollView.bounds.size.width) > scrollView.bounds.size.width / 2 {
