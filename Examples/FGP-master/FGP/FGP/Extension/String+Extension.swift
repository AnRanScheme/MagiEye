...
extension String {
    ...
    func stringCut(end: Int) ->String{
        ...
        return String(describing: [..<sInde])
    }
    ...
    func stringCutToEnd(star: Int) -> String {
        ...
        return String(describing: [sRang])
    }
    ...
    func stringInsert(content: String,locat: Int) -> String {
        if !(locat < self.count) { return "截取超出范围" }
    ...
    func  getStringSize(text: String, rectSize: CGSize,fontSize: CGFloat) -> CGSize {
        ...
        let rect = str.boundingRect(with: rectSize, options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: [.font: UIFont.systemFont(ofSize: fontSize)], context: nil)
